<template>
	<view class="box">
		<view class="title">党建翼联总结审批</view>
		<view class="uni-form">
			...
			<view class="uni-form-item">
				<view class="label">本支部实际参加人数</view>
				<view class="content">
					{{deatailInfo.realNum}}
				</view>
			</view>
			<view class="uni-form-item">
				<view class="label">对象支部实际参加人数</view>
				<view class="content">
					{{deatailInfo.objBranchActualParticipants}}
				</view>
			</view>
			<view class="uni-form-item">
				<view class="label">简要总结</view>
				<view class="content">
					{{deatailInfo.briefSummary}}
				</view>
			</view>
			<view class="uni-form-item">
				<view class="label">党员分享</view>
			...
			<view class="uni-form-item">
				<view class="label">特色亮点</view>
				<view class="content">{{deatailInfo.specialHighlights}}</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			async queryInfo() {
				...
				this.imgList = this.deatailInfo.isAcceptShareList.map(item=> ({
			...
			async submit(type) {
				...
				const { data } = await this.$http({
					url: `/party/activity/audit/${this.dataId}/${this.todoId}/${auditType}`,
					method: 'GET'
				});
				if (data.code === 200) {
					uni.showToast({
